...
|   11\*\*    |    NTB \# 2, 3    |   \# 1, 2, 9\*\*\*, 17\*\*\*, 18\*\*\*, 20   |
|     12      |    NTB \# 4, 5    | \# 1, 2, 7, 11, 12, 15, 19, 25, 27, NTB \# 6 |
...
3.  There is a bowl containing 30 cashews, 20 pecans, 25 almonds, and 25 walnuts. I am going to randomly pick and eat 3 nuts (without replacement). Find the expected value of the number of cashews by defining the number of cashews as a sum of random variables. (This one takes a little while if we don't rely on the

4.  Prove that for a r.v. $X$ and constants $a$ and $b$, that $$\mathrm{Var}[aX+b]=a^2\mathrm{Var}[X].$$

5.  Let $\bar{X}$ be the random variable for the sample mean, $\bar{X}=\frac{\sum_{i=1}^nX_i}{n}$, where the $X_i$ are i.i.d. random variables with common mean $\mu$ and variance $\sigma^2$.
